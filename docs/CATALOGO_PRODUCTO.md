...
| App de escritorio Windows (`launcher.py` + `app_desktop/` + `installer/`) | distribución de escritorio | ✅ [**binario publicado en release v3.11.0**](https://github.com/vladimiracunadev-create/python-data-science-program/releases/tag/v3.11.0) (ZIP portable slim, 274 MB) | alumno / docente en aula | **app Qt nativa con PySide6 — sin web, sin localhost, sin WebView, sin Flask de fondo**; QTreeView con las 9 partes + 232 clases, `QTextBrowser.setHtml` alimentado por `app/class_html.py` (el mismo conversor que genera GitHub Pages) con resaltado Pygments, render por celda para los notebooks (outputs PNG base64 → QPixmap). Sin Python instalado en el equipo del usuario. Bundle slim: PDFs/PPTX no van empaquetados — el viewer abre la URL raw de GitHub cuando el archivo local no existe. |
| App Android (`mobile/`) | distribución móvil | ✅ [**APK debug v3.11.0 publicado en release**](https://github.com/vladimiracunadev-create/python-data-science-program/releases/tag/v3.11.0) (versionCode 42) · **232 clases embebidas** (verificadas dentro del binario) | alumno en movimiento | catálogo generado desde `classes/**/README.md` (`scripts/generate_mobile_curriculum.py`), validado en `tests/test_mobile_curriculum.py`; navegación jerárquica partes → clases → detalle, con buscador y progreso por parte |
...
## 🚀 Siguiente paso (post-v3.11.0)

🎓 **El currículo está 100% real (v3.11.0: 232/232 README + 232/232 notebooks ejecutables + 232/232 PDFs + 232/232 PPTX).** El release v3.11.0 publicó el ZIP portable de la app Windows —con la clase renderizada en HTML— y el APK Android reconstruido (versionCode 42) con el bundle JS al día. Lo que queda:
