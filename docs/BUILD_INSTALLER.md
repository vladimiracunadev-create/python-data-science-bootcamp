...
Genera el ejecutable Windows del Python Data Science Program como **aplicación de escritorio Qt nativa** (v3.8.0+).
...
```
release_artifacts/
  PythonDSProgram_windows_portable_v3.8.0.zip   ← portable slim (~274 MB; descomprimir y ejecutar)

dist_installer/
  PythonDSProgram_Setup_v3.8.0.exe              ← instalador opcional (Inno Setup)
...
PythonDSProgram.exe
    ...
    release_artifacts/PythonDSProgram_windows_portable_v3.8.0.zip

    ↓ Inno Setup (installer/setup.iss)  — opcional
    dist_installer/PythonDSProgram_Setup_v3.8.0.exe
```
...
Compress-Archive -Path "dist\PythonDSProgram\*" `
  -DestinationPath "release_artifacts\PythonDSProgram_windows_portable_v3.8.0.zip" -Force
...
Compartir `release_artifacts/PythonDSProgram_windows_portable_v3.8.0.zip` (~274 MB).
El alumno descomprime y ejecuta `PythonDSProgram.exe` directamente.

**Opción instalador:**

Compartir `dist_installer/PythonDSProgram_Setup_v3.8.0.exe`.
