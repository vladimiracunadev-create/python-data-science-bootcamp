...
> **Estado v3.9.0:** la app embebe las **232 clases en 9 partes**. `mobile/src/data/classes.js` se genera desde `classes/**/README.md` con `python scripts/generate_mobile_curriculum.py` y se valida en `tests/test_mobile_curriculum.py` — no se edita a mano. El APK se **reutiliza sin cambios desde v3.8.1** (mismo catálogo; los enlaces a Colab ahora resuelven a notebooks reales).
>
