# Clase 126 — Transformers: arquitectura, BERT, GPT (+ Flash Attention, RoPE, GQA)
...
## 📌 Complemento: Flash Attention, RoPE, GQA — los engranajes de los LLMs 2024-2026

### Flash Attention (Dao et al. 2022, v2 2023, v3 2024)

El cuello de botella de attention es **memoria**, no compute. Calcular `softmax(QK^T)V` requiere materializar la matriz `(N, N)` en VRAM — para N=8192, son 64 MB por head con `float16`, multiplicado por num_heads * num_layers * batch. **Flash Attention** reformula el cálculo en **bloques con online softmax**, manteniendo solo bloques pequeños en SRAM:

- Memoria: **O(N) en lugar de O(N²)**.
- Velocidad: **2-3× speedup** en wall-clock vs implementación naïve.
- v3 (2024): optimizado para H100/Hopper, ~75 % de FLOPS teóricos.

```python
# En PyTorch 2.0+
output = torch.nn.functional.scaled_dot_product_attention(q, k, v, is_causal=True)
# Se elige automáticamente el backend más rápido: Flash, Memory-Efficient, o math.
```

Lo mismo está en `transformers` library, llave en mano. Si entrenás un Transformer desde cero hoy, **siempre Flash Attention**.

### RoPE — Rotary Position Embeddings (Su et al. 2021, adoptado por Llama)

Positional encoding clásico (sinusoidal o aprendible) **suma** una representación de posición al embedding. RoPE en su lugar **rota** los Q y K en pares de dimensiones por un ángulo que depende de la posición:

```python
# Pseudocódigo
def apply_rope(q, position_ids):
    # q: (batch, heads, seq, dim_per_head)
    sin, cos = compute_sin_cos(position_ids)
    # Rotar pares de dimensiones (q[..., 0::2], q[..., 1::2])
    return q_rotated
```

**Ventajas**:

- Propiedad **relativa**: `attention(q_i, k_j)` depende solo de `i - j`, no de la posición absoluta.
- **Extrapolación**: modelos entrenados con `T=2048` pueden manejar `T=8192` con técnicas como **NTK scaling** o **YaRN**.
- Es la elección de Llama, Mistral, Falcon, Qwen — prácticamente todo LLM open-source moderno.

### Grouped-Query Attention (GQA) — Ainslie et al. 2023

Multi-head attention tiene `H` heads para Q, K, V. **KV cache** en inference de LLMs crece con `H * d_head * seq_len * batch` → bottleneck de memoria con secuencias largas.

- **MHA (clásico)**: `H_q = H_kv`. Más expresivo, más KV.
- **MQA (Multi-Query)**: `H_kv = 1`. Mínimo KV, pero pérdida de calidad.
- **GQA**: `H_kv ∈ {1, 2, 4, ..., H_q/2}`. Compromiso — Llama 2 70B usa 8 KV heads para 64 query heads.

Resultado: **inference más rápido y memory-efficient** sin perder calidad. Default en LLMs modernos.

### Combinación moderna (un Transformer 2026)

```
Block:
    LayerNorm  (RMSNorm — más rápida que LayerNorm clásica)
    GQA con RoPE     ← Flash Attention v3 por debajo
    Residual
    LayerNorm
    SwiGLU FFN       ← variante de GELU más eficiente
    Residual
```

Esa receta es lo que está dentro de Llama 3, Mistral, Mixtral, Qwen, Gemma. Saber TF/Keras lo cubre conceptualmente; en producción, implementación via `transformers` library.
...
[Clase 127 — Hugging Face Transformers (uso práctico)](../127-hugging-face-transformers-uso-practico/README.md)
