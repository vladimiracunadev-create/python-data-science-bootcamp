# Clase 102 — Optimizadores: Momentum, Nesterov, AdaGrad, RMSProp, Adam, AdamW (+ Lion, Sophia)
...
## 📌 Complemento: Lion y Sophia — los optimizadores 2023+ para LLMs

A medida que los modelos escalan (Llama 3, GPT-4, Mistral), entrenar con Adam se vuelve caro: requiere mantener **dos buffers** del tamaño del modelo (`m`, `v`) → para un modelo de 70B, eso es 280 GB extra de VRAM. Aparecieron alternativas que son más rápidas y/o usan menos memoria:

### Lion (EvoLved Sign Momentum, Chen et al. 2023)

```
m_t = β₁ · m_{t-1} + (1 - β₁) · g_t          # EMA del gradiente
w_t = w_{t-1} - η · sign(β₂ · m_{t-1} + (1 - β₂) · g_t)   # paso = signo
```

Lion solo usa el **signo** del gradiente (–1 / 0 / +1) → todos los pasos tienen la misma magnitud `η`. Esto:

- Reduce buffer a uno solo (`m`) → memoria 50 % vs Adam.
- En benchmarks (Chen 2023): supera Adam en ViT, CLIP, BERT y modelos de difusión a tamaño grande.
- **LR es 3-10× más chico** que en Adam: si Adam usaba `1e-3`, Lion suele querer `1e-4` o `3e-4`.
- Weight decay 3-10× **más grande** que en AdamW.

```python
optimizer = keras.optimizers.Lion(learning_rate=1e-4, weight_decay=0.1, beta_1=0.9, beta_2=0.99)
```

### Sophia (Second-order Optimizer with Hessian, Liu et al. 2023)

Aproxima información de **segundo orden** (Hessiano) sin pagar el costo `O(n²)`:

- Estima la diagonal del Hessiano con muestreo de Hutchinson.
- Divide el gradiente por esa estimación → pasos más uniformes en todas las direcciones.
- En el paper: **2× speedup** vs AdamW en pre-training de LLMs de hasta 770M (GPT-2 scale).

```python
# Implementación en optax (JAX) o package separado en TF
# from sophia import SophiaG
# optimizer = SophiaG(lr=1e-4, betas=(0.965, 0.99), rho=0.04)
```

Sophia aún no está en Keras nativo (a 2026), pero hay implementaciones de la comunidad y está en `optax` para JAX. Vale la pena seguirlo: si entrenás un LLM mediano desde cero, te puede ahorrar la mitad del cómputo.

### Cuál usar — guía 2026

| Caso | Recomendación |
|---|---|
| MLP / CNN clásica, dataset moderado | **AdamW** (LR=1e-3, weight_decay=1e-2) |
| Fine-tuning de modelos preentrenados | **AdamW** (LR=1e-5 a 1e-4) |
| Visión grande desde cero (ImageNet, ViT) | **AdamW** o **Lion** |
| Pretraining de LLMs desde cero | **AdamW** con cosine schedule; **Lion**/**Sophia** si la budget de memoria/cómputo es limitada |
| Fine-tuning RL (PPO, etc.) | **Adam** clásico (no AdamW — weight decay puede degradar) |

> **Cuidado con el legacy**: Keras antes tenía `Adam(weight_decay=...)` que aplicaba weight decay como L2 regularization (incorrecto). Usá `keras.optimizers.AdamW` explícitamente.
...
[Clase 103 — Learning rate scheduling](../103-learning-rate-scheduling/README.md)
