# Clase 153 — Bootstrap y permutation tests
...
## 📌 Complemento: APIs modernas de scipy + BCa bootstrap

Hasta scipy < 1.7, el bootstrap había que escribirlo a mano (loop, `rng.choice(arr, size=n, replace=True)`). Desde **scipy 1.9** la API `scipy.stats.bootstrap` es la forma canónica — vectorizada, con tres métodos de IC, y soporte para estadísticos multivariados.
...
x = rng.lognormal(mean=0, sigma=1, size=200)  # asimétrico, t-test fallaría
res = bootstrap(
    (x,),                       # tupla de samples (uno por argumento del statistic)
    statistic=np.median,        # cualquier callable
    n_resamples=10_000,
    method='BCa',               # 'percentile' | 'basic' | 'BCa' (default desde 1.10)
    confidence_level=0.95,
    random_state=rng,
)
print(res.confidence_interval)  # ConfidenceInterval(low=..., high=...)
print(res.standard_error)
```

**¿Por qué BCa?** El bootstrap percentile asume que la distribución bootstrap es simétrica respecto al estimador. Si hay **sesgo** (mediana en datos asimétricos) o **aceleración** (la varianza del estimador depende del parámetro), percentile sub-cubre. **BCa** (Efron 1987) corrige ambas cosas:

1. **Bias correction (z₀)**: estima cuánto se aleja el promedio bootstrap del estimador original.
2. **Acceleration (a)**: estima cómo cambia el SE con el parámetro, usando jackknife.
3. Ajusta los percentiles que se toman: en vez de `2.5, 97.5`, usa `α₁, α₂` corregidos.

Resultado: **cobertura más cercana a la nominal** (~ 95 %), especialmente con muestras chicas o estadísticos no lineales. Es el default moderno; usá BCa salvo que tengas razón específica para no.

**Permutation test moderno**:

```python
from scipy.stats import permutation_test

def diff_means(a, b, axis=0):
...
res = permutation_test(
    ...
    permutation_type='independent',   # 'samples' (pareadas) o 'pairings' (correlación)
    random_state=rng,
)
print(res.statistic, res.pvalue)
```

Tres tipos de permutación:

- `'independent'`: dos muestras independientes (concatena y permuta). Equivalente moderno de Mann-Whitney sin asumir nada sobre la distribución.
- `'samples'`: pareadas — intercambia entre pares. Equivalente a Wilcoxon signed-rank.
- `'pairings'`: correlación / asociación — permuta una columna respecto a la otra. Equivalente a test de Spearman sin asumir nada.

**Cuándo bootstrap, cuándo permutación**:

- **Bootstrap** → IC, SE, sesgo de un estimador. Trabaja en **una** muestra.
- **Permutación** → p-value de comparación entre **dos o más** grupos bajo `H₀` de "no diferencia". Es exacto (condicional a los datos), no asintótico.

Para una comparación de dos grupos con datos raros, lo ideal: reportar p-value por **permutation_test** + IC de la diferencia por **bootstrap BCa**. Cero supuestos paramétricos, máxima robustez.
...
[Clase 154 — A/B testing: tamaño de muestra, poder estadístico](../154-a-b-testing-tamano-de-muestra-poder-estadistico/README.md)
